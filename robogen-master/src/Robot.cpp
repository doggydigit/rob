...
bool Robot::init(dWorldID odeWorld, dSpaceID odeSpace,
	...
	std::cout<<"Robot.cpp/init/ begin" << std::endl;
	...
	std::cout<<"Robot.cpp/init/ done" << std::endl;
...
bool Robot::decodeBody(const robogenMessage::Body& robotBody) {
	std::cout<<"Robot.cpp/decodeBody/ begin" << std::endl;
	...
	std::cout<<"---Robot.cpp/decodeBody/ done" << std::endl;

	for(unsigned int i=0; i<sensors_.size();++i)
	{
		if (sensors_[i]->getCamera())
		{
			Camera = sensors_[i];
			return true;
		}
...
bool Robot::decodeBrain(const robogenMessage::Brain& robotBrain) {
	...
	::initNetwork(neuralNetwork_.get(), nInputs, nOutputs, &weight[0], &bias[0],
			&gain[0]);
	std::cout<<"---Robot.cpp/decodeBrain/ done" << std::endl;
...
void Robot::reconnect() {
	...
	std::cout<<"---Robot.cpp/reconnect/connect done" << std::endl;
...
void Robot::translateRobot(const osg::Vec3& translation) {
	std::cout<<"---Robot.cpp/translateRobot/ begin" << std::endl;

	for (unsigned int i = 0; i < this->bodyParts_.size(); ++i) {
		this->bodyParts_[i]->translateRootPosition(translation);
	}
	std::cout<<"---Robot.cpp/translateRobot/ translate done" << std::endl;
...
//boost::shared_ptr<Model> & Robot::getCam(){return Cam}
